...
def upload_image():
    ...
    if operation_selection == 'gray':
    	...
    	file_data = indianoil(decode_array_to_img)
    elif operation_selection == 'rgb':
    	file_data = bgr2rgb(decode_array_to_img)
...
def indianoil(decode_array_to_img):
	indianoil_effect = cv2.xphoto.oilPainting(decode_array_to_img, 7, 1)
	status, output_img = cv2.imencode('.PNG', indianoil_effect)

	return output_img

def bgr2rgb(decode_array_to_img):
	bgr2rgb_converter = cv2.cvtColor(decode_array_to_img, cv2.COLOR_BGR2RGB)
	status, output_img = cv2.imencode('.PNG', bgr2rgb_converter)
